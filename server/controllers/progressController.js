...
// POST /api/progress/posttest
// Saves post-test (Java Code Challenge) results, including a per-difficulty
// breakdown (Easy/Medium/Hard/Extreme) alongside the per-topic breakdown.
// Unlike the pre-test, students may retake the post-test — each attempt is
// stored so an instructor can see improvement over time.
async function savePosttest(req, res) {
  const user_id = req.user.id
  const { topicScores, difficultyScores, weakTopics, totalScore, totalItems } = req.body

  try {
    await db.query(
      `INSERT INTO posttest_results
       (user_id, total_score, total_items, topic_scores, difficulty_scores, weak_topics, taken_at)
       VALUES (?, ?, ?, ?, ?, ?, NOW())`,
      [
        user_id,
        totalScore,
    ...
    // Award the "Quest Cleared" badge the first time a student finishes it
    const [badge] = await db.query(
      "SELECT id FROM badges WHERE badge_key = 'first-clear'"
    )
    if (badge.length > 0) {
    ...
    console.error('Post-test save error:', err)
    res.status(500).json({ message: 'Error saving post-test results.' })
  }
}

// GET /api/progress/posttest
// Returns the student's most recent post-test result
async function getPosttestResult(req, res) {
  const user_id = req.user.id
  try {
    const [rows] = await db.query(
      'SELECT * FROM posttest_results WHERE user_id = ? ORDER BY taken_at DESC LIMIT 1',
      [user_id]
    )
    if (rows.length === 0) return res.json({ taken: false })
    ...
    res.json({
      ...
      totalItems:       result.total_items,
      topicScores:      JSON.parse(result.topic_scores),
      difficultyScores: JSON.parse(result.difficulty_scores || '{}'),
      weakTopics:       JSON.parse(result.weak_topics),
      takenAt:          result.taken_at
    })
  } catch (err) {
    console.error(err)
    res.status(500).json({ message: 'Error fetching post-test result.' })
  }
}
